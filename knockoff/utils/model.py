...
def train_step(model, train_loader, criterion, optimizer, epoch, device, log_interval=10, writer=None, clip = None):
    model.train()
    train_loss = 0.
    correct = 0
    total = 0
...
def train_model(model, trainset, out_path, batch_size=64, criterion_train=None, criterion_test=None, testset=None,
    ...
    for epoch in range(start_epoch, epochs + 1):
        train_loss, train_acc = train_step(model, train_loader, criterion_train, optimizer, epoch, device,
                                           log_interval=log_interval,clip=clip)
...
def create_mask(model, range1, range2, eps):
    mask = []
    for param in model.parameters():
        if param.requires_grad:
            weights = param.data.view(-1)
            mask1 = (weights >= range1 - eps) & (weights <= range1 + eps)
            mask2 = (weights >= range2 - eps) & (weights <= range2 + eps)
...
def train_model_with_mask(model, trainset, out_path, range1, range2, eps,batch_size=64, criterion_train=None, criterion_test=None, testset=None,
                device=None, num_workers=10, lr=0.1, momentum=0.5, lr_step=30, lr_gamma=0.1, resume=None,
                epochs=100, log_interval=100, weighted_loss=False, checkpoint_suffix='', optimizer=None, scheduler=None,
                writer=None, **kwargs):
    if device is None:
    ...
    model_out_path = osp.join(out_path, 'checkpoint{}.pth.tar'.format(checkpoint_suffix))
    mask = create_mask(model, range1, range2, eps)
