...
class Blackbox(object):
    ...
    def from_modeldir(cls, model_dir, device=None, output_type='probs'):
        ...
        if not os.path.exists(params_path):
            params_path = osp.join(model_dir, 'params_train.json')
        assert osp.exists(params_path)
    ...
    @classmethod
    def load_checkpoint(cls, model, target_dir, ckpt_name=None):
        # Load weights
        if ckpt_name is None:
            checkpoint_path = osp.join(target_dir, 'model_best.pth.tar')
            if not osp.exists(checkpoint_path):
                checkpoint_path = osp.join(target_dir, 'checkpoint.pth.tar')
        else:
            checkpoint_path = osp.join(target_dir, ckpt_name)
