...
class mod_conv(nn.Conv2d):
    def __init__(self, *args, restore_params=None, **kwargs):
...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, img_size=32, pretrained=False,restore_params=None):
...
def make_layers(cfg,restore_params=None,batch_norm=False):
    layers = []
    in_channels = 3
    i = 0
    if restore_params==None:
        ...
        for v in cfg:
            if v == 'M':
                ...
                conv2d = mod_conv(in_channels, v, restore_params = restore_params[i],kernel_size=3, padding=1)
                i+=1
                if batch_norm:
                    layers += [conv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
                else:
...
def vgg16_bn(pretrained=False,restore_params=None, **kwargs):
    """VGG 16-layer model (configuration "D") with batch normalization"""
    model = VGG(make_layers(cfg['D'], batch_norm=True,restore_params=restore_params),restore_params=restore_params, **kwargs)
    if pretrained:
...
def vgg19_bn(pretrained=False, **kwargs):
    ...
    # Forward blocks:  ['features.0', 'features.3', 'features.7', 'features.10', 'features.14', 'features.17', 'features.20', 'features.23', 'features.27', 'features.30', 'features.33', 'features.36', 'features.40', 'features.43', 'features.46', 'features.49', 'last_linear', 'end']
    # Backward blocks:  ['last_linear', 'features.49', 'features.46', 'features.43', 'features.40', 'features.36', 'features.33', 'features.30', 'features.27', 'features.23', 'features.20', 'features.17', 'features.14', 'features.10', 'features.7', 'features.3', 'features.0', 'start']
