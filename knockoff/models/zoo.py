...
import knockoff.models.face
import knockoff.models.imagenet
from pdb import set_trace as st

def get_net(modelname, modeltype, pretrained=None, restore_params=None,**kwargs):
    assert modeltype in ('mnist', 'cifar', 'imagenet', 'face')
    # print('[DEBUG] pretrained={}\tnum_classes={}'.format(pretrained, kwargs['num_classes']))
    if restore_params and restore_params is not None:
        model = eval('knockoff.models.{}.{}'.format(modeltype, modelname))(restore_params = restore_params, **kwargs)
        print(111)
        return model
...
def get_pretrainednet(modelname, modeltype, pretrained='imagenet', num_classes=1000, **kwargs):
    ...
    elif pretrained == 'imagenet_for_cifar':
        return get_imagenet_for_cifar_pretrainednet(modelname, num_classes, **kwargs)
    elif pretrained == 'imagenet_for_face':
        return get_imagenet_for_face_pretrainednet(modelname, num_classes, **kwargs)
...
def get_imagenet_pretrainednet(modelname, num_classes=1000, **kwargs):
    ...
    if num_classes != 1000 and "resnet" not in modelname:
        # Replace last linear layer
        in_features = model.last_linear.in_features
        out_features = num_classes
        model.last_linear = nn.Linear(in_features, out_features, bias=True)
    elif num_classes != 1000 and "resnet" in modelname:
        in_features = model.fc.in_features
        out_features = num_classes
        model.fc = nn.Linear(in_features, out_features, bias=True)
    return model

def get_imagenet_for_cifar_pretrainednet(modelname, num_classes=1000, **kwargs):
    valid_models = knockoff.models.cifar.__dict__.keys()
    print(valid_models)
    assert modelname in valid_models, 'Model not recognized, Supported models = {}'.format(valid_models)
    model = knockoff.models.cifar.__dict__[modelname](pretrained=True, num_classes=num_classes)
    return model

def get_imagenet_for_face_pretrainednet(modelname, num_classes=1000, **kwargs):
    valid_models = knockoff.models.face.__dict__.keys()
    assert modelname in valid_models, 'Model not recognized, Supported models = {}'.format(valid_models)
    model = knockoff.models.face.__dict__[modelname](pretrained=True, num_classes=num_classes)
    return model
