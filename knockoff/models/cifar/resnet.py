...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152', 'swsl_resnet18', 'ssl_resnet18', 'gluon_resnet18',
'quant_resnet18' ]
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, img_size=32, attack_backbone=False):
        self.inplanes = 64
...
def quant_resnet18(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    ...
    model = ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        path = 'https://download.pytorch.org/models/quantized/resnet18_fbgemm_16fa66dd.pth'
        ckp = model_zoo.load_url(path)
        state_dict = model.state_dict()
        if ckp['fc.weight'].size(0) != state_dict['fc.weight'].size(0):
            ckp['fc.weight'] = state_dict['fc.weight']
            ckp['fc.bias'] = state_dict['fc.bias']
