...
def modify_conv_layers(model, modify_ratio=0.2):
    ...
    for layer_name, layer in modified_model.named_modules():
        if isinstance(layer, nn.Conv2d):
            ...
            num_channels_to_shuffle = int(in_channels * modify_ratio)
            perm = torch.randperm(num_channels_to_shuffle).cuda()
            shuffle_indices[f'{layer_name}'] = perm.cpu().numpy()
            layer.weight.data[:, :num_channels_to_shuffle, :, :] = layer.weight.data[:, perm, :, :]
...
def main():
    ...
    print('Secret soter model: [Test]  Epoch: {}\tAcc: {:.1f}% ({}/{})'.format(epoch, acc,
                                                                             correct, total))
