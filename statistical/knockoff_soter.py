...
def find_max_similarity(pre_model, modified_model):
    ...
    for layer_name, pre_layer in pre_model.named_modules():
        if isinstance(pre_layer, nn.Conv2d):
            ...
            num_kernels_to_process = int(in_channels * 0.2)
            
            max_sim_index = []
            pre_weights = torch.zeros((num_kernels_to_process, out_channels * kernel_height * kernel_width)).cuda()
            mod_weights = torch.zeros((num_kernels_to_process, out_channels * kernel_height * kernel_width)).cuda()

            for i in range(num_kernels_to_process):
...
def calculate_weight_ratios(pre_model, model):

    weight_ratios = {}

    for (pre_name, pre_layer), (name, layer) in zip(pre_model.named_modules(), model.named_modules()):
        if isinstance(pre_layer, nn.Conv2d) and isinstance(layer, nn.Conv2d):
            assert pre_name == name and pre_layer.weight.shape == layer.weight.shape

            pre_layer_avg_weight = pre_layer.weight.data.mean().item()
            layer_avg_weight = layer.weight.data.mean().item()

            if pre_layer_avg_weight != 0:
                ratio = layer_avg_weight / pre_layer_avg_weight
            else:
                ratio = float('inf')  
            weight_ratios[pre_name] = ratio
...
def restore_shuffling(modified_model, de_shuffle):
    for layer_name, layer in modified_model.named_modules():
        if isinstance(layer, nn.Conv2d):
            shuffle_indices = de_shuffle[layer_name]
            re_shuffle = inverse_permutation(shuffle_indices)

            out_channels, in_channels, kernel_height, kernel_width = layer.weight.shape
            num_kernels_to_restore = int(in_channels * 0.2)
            with torch.no_grad():
                restored_weights = torch.zeros_like(layer.weight)
                for i in range(num_kernels_to_restore):
                    restored_index = re_shuffle[i]
                    restored_weights[:, i, :, :] = layer.weight[:, restored_index, :, :]
                if num_kernels_to_restore < in_channels:
                    restored_weights[:, num_kernels_to_restore:, :, :] = layer.weight[:, num_kernels_to_restore:, :, :]
                layer.weight.data = restored_weights

    return modified_model

def restore_scaling(pre_model, modified_model, scale_factor=3.5, re_factor=0.3):
    pre_model = pre_model.cuda()
    modified_model = modified_model.cuda()
    ...
    for layer_name, pre_layer in pre_model.named_modules():
        if isinstance(pre_layer, nn.Conv2d):
            ...
            for i in range(in_channels):
                ...
                if abs(mod_mean*ra) > scale_factor * abs(pre_mean):
                    with torch.no_grad():
...
def main():
    ...
    parser.add_argument('--re_scale_factor', type=float, default=0.3, metavar='N', help='recover rate')
    parser.add_argument('--assume_scale_factor', type=float, default=3.5, metavar='N', help='params > this rate to be scaled')
    ...
    modified_model = zoo.get_net(model_name, modelfamily,pretrained, num_classes=num_classes)
    ...
    print('soter model: [Test]  Epoch: {}\tAcc: {:.1f}% ({}/{})'.format(epoch, acc,
                                                                             correct, total))
    ...
    print('de-shuffle model without budget: [Test]  Epoch: {}\tAcc: {:.1f}% ({}/{})'.format(epoch, acc,
                                                                             correct, total))
    # if(params['re_scale_factor']==0.5):
    #     params['re_scale_factor'] = calculate_weight_ratios(pre_model, deshuffled_model)
    re_factor = params['re_scale_factor']
    model = restore_scaling(pre_model,deshuffled_model,re_factor=re_factor)
    total = 0
    correct = 0
    device = torch.device('cuda')
    model.to(device)
    ...
    for b in budgets:
        ...
        outputs_path = f'models/soter/log-{model_name}-{dataset_name}'
    ...
    # save_path = (f'models/soter/log/{model_name}-{dataset_name}-soter.csv')
    # log_training_results('train.log.tsv', epoch, train_loss, train_accuracy)
