...
def replace_weights_in_range(pre_model, model, range1, range2, eps):

    range1_lower, range1_upper = range1
    range2_lower, range2_upper = range2

    model_params = [param for param in model.parameters() if param.requires_grad]
    pre_model_params = [param for param in pre_model.parameters() if param.requires_grad]


    assert len(model_params) == len(pre_model_params), "Models have different number of parameters"

    with torch.no_grad():
        for model_param, pre_model_param in zip(model_params, pre_model_params):

            model_weights = model_param.data.view(-1)
            pre_model_weights = pre_model_param.data.view(-1)


            mask1 = (model_weights >= range1_lower - eps) & (model_weights <= range1_upper + eps)
            mask2 = (model_weights >= range2_lower - eps) & (model_weights <= range2_upper + eps)
            total_mask = mask1 | mask2


            model_weights[total_mask] = pre_model_weights[total_mask]
...
def main():
    ...
    print('-'*20,modified_dict['arch'],modified_dict['best_acc'])

    model.load_state_dict(modified_dict['state_dict'])
    eps=2e-3
    max_w,min_w = find_max_and_min_weights(model)
    replace_weights_in_range(pre_model, model, (min_w-eps, min_w+eps), (max_w-eps, max_w+eps), eps)
    ...
    for b in budgets:
        ...
        outputs_path = f'models/nnsplitter/log-{model_name}-{dataset_name}'
        model = model.to(device)
