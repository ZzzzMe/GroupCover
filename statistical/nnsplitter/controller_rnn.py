...
class Controller_rnn(nn.Module):
    ...
    def policy_gradient(self, arg, lmd, trainloader, testloader,net):
        '''
        ...
        for ep in range(arg.num_epoch_rl):
            ...
            for k in range(arg.batch_size_rl):
                ...
                change_acc = inference(net,device,testloader)
               ...
               'created_on': str(datetime.now())
                    ...
                    torch.save(state, arg.PATH+'.pth')
                    # torch.save(model_dict, arg.PATH)
                    np.save(arg.PATH+'_ori_w.npy', ori_w)
                    np.save(arg.PATH+'_new_w.npy', new_w)
                    best = reward
                    cnt = 0
                else:
                    cnt += 1
